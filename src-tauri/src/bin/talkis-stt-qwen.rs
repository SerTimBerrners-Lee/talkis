...
const SERVER_NAME: &str = "talkis-stt-qwen";
const ENGINE_NAME: &str = "qwen-asr";
const MAX_REQUEST_BYTES: usize = 128 * 1024 * 1024;
const ENGINE_SCRIPT_NAME: &str = "qwen_engine.py";
const QWEN_ENGINE_SCRIPT: &str = include_str!("../runtime_engines/qwen_engine.py");
...
const QWEN_MODELS: &[QwenModel] = &[QwenModel {
    id: "Qwen/Qwen3-ASR-0.6B",
    aliases: &["qwen3-asr-06b", "Qwen3-ASR-0.6B"],
    dir_name: "qwen3-asr-06b",
}];
...
fn find_model(value: &str) -> Option<&'static QwenModel> {
    QWEN_MODELS.iter().find(|model| {
...
fn model_dir(config: &RuntimeConfig, model: &QwenModel) -> PathBuf {
    config.models_dir.join(model.dir_name)
}

fn marker_path(config: &RuntimeConfig, model: &QwenModel) -> PathBuf {
    config.models_dir.join(format!("{}.json", model.dir_name))
}

fn engine_dir(config: &RuntimeConfig) -> PathBuf {
    config.data_dir.join("runtime").join("qwen-engine")
}
...
fn install_model(config: &RuntimeConfig, requested: &str) -> Result<String, (u16, String)> {
    let model = find_model(requested).ok_or_else(|| {
        (
            404,
            format!(
                "Модель «{}» не поддерживается встроенным Qwen runtime.",
                requested
            ),
        )
    })?;
    ...
    if !destination.join("config.json").is_file() {
        fs::create_dir_all(&destination).map_err(|err| {
            (
                500,
                format!("Не удалось подготовить директорию Qwen модели: {}", err),
            )
        })?;
...
fn write_model_marker(config: &RuntimeConfig, model: &QwenModel) -> Result<(), (u16, String)> {
    let marker = json!({
        "id": model.id,
...
fn transcribe(request: &HttpRequest, config: &RuntimeConfig) -> Result<String, (u16, String)> {
    ...
    fs::write(&audio_path, &multipart.file)
        .map_err(|err| (500, format!("Не удалось сохранить аудио для Qwen: {}", err)))?;
    ...
    let parsed: serde_json::Value = serde_json::from_str(&stdout).map_err(|err| {
        (
            502,
            format!("Qwen engine вернул некорректный JSON: {} ({})", err, stdout),
        )
    })?;
    Ok(parsed
        .get("text")
...
fn ensure_engine(config: &RuntimeConfig) -> Result<(), (u16, String)> {
    ...
    fs::create_dir_all(&engine_dir).map_err(|err| {
        (
            500,
            format!("Не удалось подготовить Qwen engine runtime: {}", err),
        )
    })?;

    let script_path = engine_script_path(config);
    if fs::read_to_string(&script_path).ok().as_deref() != Some(QWEN_ENGINE_SCRIPT) {
        fs::write(&script_path, QWEN_ENGINE_SCRIPT).map_err(|err| {
            (
                500,
                format!("Не удалось сохранить Qwen engine script: {}", err),
            )
        })?;
    }
    ...
    if venv_python(config).is_file() && !venv_is_usable(config) {
        fs::remove_dir_all(venv_dir(config)).map_err(|err| {
            (
                500,
                format!("Не удалось очистить поврежденный Qwen Python venv: {}", err),
            )
        })?;
    ...
    if !venv_python(config).is_file() {
        ...
        let output = Command::new(&python)
            ...
            .map_err(|err| {
                (
                    500,
                    format!("Не удалось создать Python venv для Qwen: {}", err),
                )
            })?;
        if !output.status.success() {
            return Err((
                500,
                format!(
                    "Не удалось создать Python venv для Qwen: {}",
    ...
    if !dependencies_marker.is_file() {
        ...
        run_python(
            &venv_python(config),
            ["-m", "pip", "install", "-U", "qwen-asr", "huggingface_hub"],
        )?;
        fs::write(&dependencies_marker, "ok").map_err(|err| {
            (
                500,
                format!("Не удалось сохранить состояние Qwen engine: {}", err),
            )
        })?;
    }
    ...
    I: IntoIterator<Item = String>,
{
    ...
    let output = command
        .output()
        .map_err(|err| (500, format!("Не удалось запустить Qwen engine: {}", err)))?;
    if !output.status.success() {
        return Err((
            502,
            format!(
                "Qwen engine завершился с ошибкой: {}",
                String::from_utf8_lossy(&output.stderr)
            ),
        ));
    }
    ...
    I: IntoIterator<Item = &'a str>,
{
    let output = Command::new(python)
        ...
        .map_err(|err| {
            (
                500,
                format!("Не удалось запустить Python для Qwen: {}", err),
            )
        })?;
    if output.status.success() {
        Ok(())
        ...
        Err((
            502,
            format!(
                "Не удалось установить Qwen engine dependencies: {}",
                String::from_utf8_lossy(&output.stderr)
            ),
        ))
    }
